...
const navLinks = [
  { href: "/", label: "Obras" },
  { href: "/#serie", label: "Proyecto Destacado" },
  ...
  return (
    <header
      className={cn(
        ...
        visible
          ? "top-3 translate-y-0"
          : "top-3 -translate-y-[calc(100%_+_0.75rem)]" // 0.75rem is for top-3 (12px)
      )}
    >
        ...
        <nav className="hidden md:flex items-center space-x-2 lg:space-x-4">
          {navLinks.map((link) => (
            <Link
              ...
              key={link.href}
              href={link.href}
              scroll={true} // opcional: asegura el scroll hacia el ancla
              className={cn(
